...
import { z } from "zod";
import { venueApi, VenueEvent, ApiError } from "@/lib/api";
...
export default function EditEventPage() {
  ...
  // Load the event and pre-fill form
  useEffect(() => {
    async function load() {
      try {
        const data = await venueApi.getEvents();
        const found = data.events.find((e: VenueEvent) => e.id === id);
        if (!found) {
        ...
        reset({
          ...
          doors_time: found.doors_time?.slice(0, 5) ?? "",
          stage_time: found.stage_time?.slice(0, 5) ?? "",
          ticket_url: found.ticket_url ?? "",
  ...
  if (loadError) {
    return (
      <div className="max-w-xl">
        ...
        <Button variant="secondary" className="mt-4" onClick={() => router.push("/events")}>
          Back to Events
        </Button>
      </div>
    );
  ...
  return (
    <div className="max-w-xl">
      <div className="mb-8">
        <h1 className="text-2xl font-bold tracking-tight text-[#F0EDFF]">Edit Event</h1>
        <p className="text-sm text-[#9B93C8] mt-1 truncate">{event.title}</p>
      </div>
